...
```{r}
life_historyna2 %>%
  summarize(number_nas= sum(is.na(life_historyna2)))
```
...
life_historyna2 %>%
  ...
  group_by(order) %>%
  summarize(min_lifespan_lg = min(lifespan, na.rm=TRUE),
            max_lifespan_lg = max(lifespan, na.rm=TRUE),
            mean_lifespan_lg = mean(lifespan, na.rm=TRUE),
            sd_lifespan_lg = sd(lifespan, na.rm=TRUE),
            total=n())
```
...
life_historyna2 %>%
  group_by(order) %>%
  summarize(mean_gestation_lg = mean(gestation, na.rm=TRUE),
            mean_newborn_lg = mean(newborn, na.rm=TRUE),
            mean_wean_mass_lg = mean(wean_mass, na.rm=TRUE),
            total=n()) %>%
  arrange(desc(mean_gestation_lg))
```
